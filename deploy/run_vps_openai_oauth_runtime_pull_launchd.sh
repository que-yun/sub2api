...
LOCK_DIR="${TMPDIR:-/tmp}/sub2api-vps-openai-oauth-runtime-pull.lock"
if ! mkdir "${LOCK_DIR}" 2>/dev/null; then
  echo "Another VPS OpenAI OAuth runtime pull is already running; skipping this interval."
  exit 0
fi
trap 'rmdir "${LOCK_DIR}" 2>/dev/null || true' EXIT

"${ROOT_DIR}/deploy/sync_vps_openai_oauth_runtime_to_local.sh"

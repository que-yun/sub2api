...
func (s *OpenAIGatewayService) proxyOpenAIWSHTTPBridgeTurn(
	...
	if resp.StatusCode >= 400 {
		...
		shouldFailover := s.shouldFailoverOpenAIUpstreamResponse(resp.StatusCode, upstreamMsg, respBody, account, originalModel)
		if account.Platform == PlatformGrok {
			s.handleGrokAccountUpstreamError(ctx, account, resp.StatusCode, resp.Header, respBody)
			if turn == 1 && shouldFailover {
				return nil, newOpenAIUpstreamFailoverError(resp.StatusCode, resp.Header, respBody, upstreamMsg, false, account, originalModel)
			}
	...
	for scanner.Scan() {
		...
		if eventType == "error" {
			...
			shouldFailover := s.shouldFailoverOpenAIUpstreamResponse(statusCode, errMessage, upstreamMessage, account, originalModel)
			if account.Platform == PlatformGrok {
			...
			if turn == 1 && !wroteDownstream && shouldFailover {
				return nil, newOpenAIUpstreamFailoverError(statusCode, resp.Header, upstreamMessage, errMessage, false, account, originalModel)
			}

...
func shouldUseOpenAIFailsafeTieredRouting(req OpenAIAccountScheduleRequest) bool {
	return failsafeRoutingFull() && req.GroupID != nil && !openAIFailsafeRoutingGroupExcluded(*req.GroupID)
}
...
func (s *defaultOpenAIAccountScheduler) finishLoadBalanceSelectionFallback(
	...
	if len(attempt.selectionOrder) == 0 {
		...
		return nil, candidateCount, topK, loadSkew, noAvailableOpenAISelectionError(req.RequestedModel, attempt.compactBlocked, filterStats.summary(""))
	}
	...
	return nil, candidateCount, topK, loadSkew, noAvailableOpenAISelectionError(req.RequestedModel, compactBlocked, filterStats.summary(""))
}
...
func (s *defaultOpenAIAccountScheduler) isAccountRequestCompatibleReason(ctx context.Context, account *Account, req OpenAIAccountScheduleRequest) (bool, string) {
	...
	if shouldSkipGLMMappedAccountForContextWindow(ctx, account, req.RequestedModel) {
		return false, "glm_mapped_context_window"
	}

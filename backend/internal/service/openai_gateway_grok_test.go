...
func TestPatchGrokResponsesBodyPromotesCodexAdditionalTools(t *testing.T) {
	...
	// Local convert: bare Codex shell -> exec_command function; custom/namespace dropped.
	// Upstream promotion: top-level order preserved, carrier tools appended, duplicates skipped.
	...
	require.Equal(t, "function", tools[3].Get("type").String())
	require.Equal(t, "exec_command", tools[3].Get("name").String())
...
func TestForwardGrokResponsesCodexAdditionalToolsUsesMixedCacheIntent(t *testing.T) {
	...
	// Local: agent function tools lift auto/empty tool_choice to required.
...
func TestForwardGrokResponsesClaudeDesktopClientToolsUseCacheRoute(t *testing.T) {
	...
	for i := range upstream.bodies {
		...
		// Local: free client function tools still lift empty/auto tool_choice to required.
		require.Equal(t, "required", gjson.GetBytes(upstream.bodies[i], "tool_choice").String())
